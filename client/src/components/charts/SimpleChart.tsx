          ...
          return (
            <div key={index} className={styles.barWrapper}>
              <div className={styles.barContainer}>
                ...
                >
                  <span className={styles.barValue}>£{item.value.toFixed(0)}</span>
                </div>
              </div>
              {showLabels && <span className={styles.barLabel}>{item.label}</span>}
            </div>
          );
        })}
      </div>
      ...
      {data.map((item, index) => (
        <div key={index} className={styles.horizontalBarItem}>
          <div className={styles.horizontalLabel}>
            ...
            <span className={styles.horizontalValue}>{item.value} sold</span>
          </div>
...
export const DonutChart: React.FC<DonutChartProps> = ({
  ...
  return (
    <div className={styles.donutContainer}>
      <svg viewBox="0 0 100 100" width={size} height={size} className={styles.donutSvg}>
        {segments.map((segment, index) => (
          <path
            key={index}
            d={createArcPath(segment.startAngle, segment.endAngle - 0.5, 45, 30)}
            fill={segment.color}
